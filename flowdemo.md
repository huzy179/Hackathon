...
2. Upload bộ chứng từ thương mại bằng cách kéo thả nhiều file PDF cùng lúc vào ô **Vùng kéo thả**.
   * Hệ thống tự động AI phân loại: **Invoice**, **B/L**, **Packing List**, **C/O (Certificate of Origin)**, **C/Q (Certificate of Quality/Quantity)**, **Insurance Certificate**.
...
### Bước 4: AI Processing & Kết quả kiểm tra 2 bước
1. Live Console hiển thị luồng xử lý NDJSON thời gian thực từ Backend.
2. Màn hình kết quả chia làm **2 bước**:
   * **Bước OCR Check:** Hiển thị toàn bộ dữ liệu đã bóc tách từ từng loại chứng từ (LC, Invoice, B/L, Packing List, C/O, C/Q, Insurance) theo từng tab. Chuyên viên có thể rà soát và chỉnh sửa (HITL) trực tiếp tại đây trước khi chạy Compliance Check.
   * **Bước Compliance Check — 3 Tab rõ rệt:**
     - **Tab 1: Kiểm tra nội bộ (Layer 1)**: Lỗi cấu trúc nội bộ của từng file (hóa đơn thiếu chữ ký, B/L thiếu Clean on Board, tổng tiền lệch Đơn giá × Số lượng).
     - **Tab 2: Kiểm tra chéo (Layer 2)**: Lỗi lệch giữa Invoice ↔ B/L ↔ Packing List (lệch Ngày, Trọng lượng, Số kiện).
     - **Tab 3: Đối chiếu L/C (Layer 3)**: Lỗi so khớp chứng từ với điều khoản L/C gốc.
3. Chuyên viên có thể sửa dữ liệu sai lệch qua nút bút chì (HITL). Hệ thống tự động tính toán lại và cập nhật trạng thái dòng ngay tại Client.
